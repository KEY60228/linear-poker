...
```
Session
  status: voting | needs_discussion | revealed | finalized

  voting:
    - 参加者が投票中
    - 投票済み/未投票 のみ可視（値は秘匿）
    - 全員投票完了 かつ need_info 投票者ゼロ で revealed へ自動遷移
    - 全員投票完了 かつ need_info 投票者がいる場合は needs_discussion へ自動遷移
    - 参加者の追加/削除可

  needs_discussion:
    - 全員投票完了したが need_info を選んだ人がいるため自動開票が止まっている
    - 投票内容はまだ秘匿（自分の vote は本人にだけ見える）
    - 手動 reveal ボタンで誰でも先に進められる
    - need_info 投票者が値を変えると、need_info ゼロになり次第 revealed へ自動遷移
    - 参加者の追加/削除は不可（不要に roster をいじらせない）
    - SessionList でも独立タブとして可視化される
...
sessions(
  ...
  status TEXT NOT NULL,  -- voting | needs_discussion | revealed | finalized
